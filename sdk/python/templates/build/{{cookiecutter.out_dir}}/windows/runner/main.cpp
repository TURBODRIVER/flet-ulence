...
static void ForceForegroundWindow(HWND hwnd) {
  ...
  DWORD foreground_thread_id = ::GetWindowThreadProcessId(foreground, nullptr);

  // Temporarily attach input processing so SetForegroundWindow is allowed
  // to succeed regardless of which process currently owns focus.
  bool attached = false;
  if (foreground_thread_id != current_thread_id) {
    attached = ::AttachThreadInput(current_thread_id, foreground_thread_id, TRUE) != 0;
  }

  // Cycle topmost state to force the window above all others, then drop
  // the topmost flag again so it behaves like a normal window afterward.
...
int APIENTRY wWinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE prev,
  ...
  ForceForegroundWindow(window.GetHandle());
