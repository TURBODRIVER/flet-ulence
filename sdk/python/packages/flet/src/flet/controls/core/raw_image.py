...
class RawImage(LayoutControl):
    ...
    encode/decode entirely and, on local transports (desktop app),
    are displayed from raw RGBA pixels without any image encoding or
    decoding. This makes it suitable for animations, generated
    ...
    - **Remote transports**: Pillow and NumPy frames are PNG-encoded off the
      event loop to save bandwidth; raw pixel frames for which no
      straight-alpha source is available are sent uncompressed.
    ...
    **Premultiplied alpha.** The raw-pixel path uploads frames directly as
    ...
    sent while the client is disconnected (network loss), and without it
    a `while True: await raw_image.render(...)` loop would wait for the
    lost ack forever.
    Raise it when streaming very large frames to slow remote clients,
