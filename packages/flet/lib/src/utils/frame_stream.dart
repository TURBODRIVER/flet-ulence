...
///
/// While the desktop window is minimized the Flutter frame pipeline is
/// suspended: `setState` schedules a frame that is never produced,
/// so `addPostFrameCallback` callbacks never run. The frame producer,
/// meanwhile, keeps streaming.
/// If the widget kept decoding frames and deferring old-image disposal to
/// post-frame callbacks, decoded [ui.Image]s and pending disposals would
/// pile up unbounded in the Dart heap and flush into the engine all at once
/// on resume, flooding it with exceptions.
///
